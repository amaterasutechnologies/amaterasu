...
#if ENABLE_ZMQ
#include "zmq/zmqnotificatioamaterasuerface.h"
#endif
...
#if ENABLE_ZMQ
static CZMQNotificatioamaterasuerface* pzmqNotificatioamaterasuerface = NULL;
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("amaterasu-shutoff");
...
#if ENABLE_ZMQ
    if (pzmqNotificatioamaterasuerface) {
        Unregistervalidationinterface(pzmqNotificatioamaterasuerface);
        delete pzmqNotificatioamaterasuerface;
        pzmqNotificatioamaterasuerface = NULL;
    }
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "amaterasu.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "amaterasud.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the amaterasu and zECA money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in amaterasu/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in amaterasu/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, amaterasu, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in amaterasu/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all amaterasu specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ecastake=<n>", strprintf(_("Enable or disable staking functionality for amaterasu inputs (0-1, default: %u)"), 1));
...
//    strUsage += "  -anonymizeamaterasuamount=<n>     " + strprintf(_("Keep N amaterasu anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The amaterasu Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The amaterasu Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("amaterasu-loadblk");
...
/** Sanity checks
 *  Ensure that amaterasu is running in a usable environment with all
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. amaterasu is shutting down."));
...
#endif
    // Make sure only a single amaterasu process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. amaterasu is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("amaterasu version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
#if ENABLE_ZMQ
    pzmqNotificatioamaterasuerface = CZMQNotificatioamaterasuerface::CreateWithArguments(mapArgs);

    if (pzmqNotificatioamaterasuerface) {
        Registervalidationinterface(pzmqNotificatioamaterasuerface);
    }
#endif

    // ********************************************************* Step 7: load block chain

    //amaterasu: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            ...
            try {
                ...
                //amaterasu specific: zerocoin and spork DB's
                ...
                // amaterasu: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of amaterasu") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart amaterasu to complete") << "\n";
...
//    }
//
//    nAnonymizeamaterasuAmount = GetArg("-anonymizeamaterasuamount", 0);
//    if (nAnonymizeamaterasuAmount > 999999) nAnonymizeamaterasuAmount = 999999;
//    if (nAnonymizeamaterasuAmount < 2) nAnonymizeamaterasuAmount = 2;
    ...
    LogPrintf("Anonymize amaterasu Amount %d\n", nAnonymizeamaterasuAmount);
