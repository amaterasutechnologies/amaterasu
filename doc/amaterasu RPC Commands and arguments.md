# amaterasu RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, amaterasu-qt (or amaterasud) will look for a file named amaterasu.conf in the amaterasu data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.amaterasu/amaterasu.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/amaterasu.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\amaterasu\amaterasu.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\amaterasu\amaterasu.conf |

Note: if running amaterasu in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of amaterasu Core version 2.0.0:
...
Start amaterasu Core Daemon with option:
<table>
<tr> <td>amaterasud</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: amaterasu.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: amaterasud.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the amaterasu and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in amaterasu/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in amaterasu/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all amaterasu specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for amaterasu inputs (0-1, default: 1)
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | amaterasu )
...
#### preparebudget "proposal-name" "url" payment-count block-start "amaterasu-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "amaterasu-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or amaterasu.conf setting gen) It can also be set with the setgenerate call.
...
#### validateaddress “amaterasuaddress”

Return information about the given amaterasu address.

#### verifymessage “amaterasuaddress” “signature” “message”
...
#### bip38decrypt "amaterasuaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "amaterasuaddress" "passphrase"

Encrypts a private key corresponding to 'amaterasuaddress'.

#### dumpprivkey “amaterasuaddress”

Reveals the private key corresponding to ‘amaterasuaddress’. Then the importprivkey can be used with this output
...
DEPRECATED. Returns the current amaterasu address for receiving payments to this account.
...
Returns a new amaterasu address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new amaterasu address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “amaterasuaddress” ( minconf addlockconf )

Returns the total amount received by the given amaterasuaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “amaterasuprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “toamaterasuaddress” amount ( minconf addlockconf “comment” “comment-to” )

DEPRECATED (use sendtoaddress). Sent an amount from an account to a amaterasu address.
...
#### sendtoaddress “amaterasuaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending amaterasus
...
Usage:
  amaterasu-tx [options] <hex-tx> [commands]  Update hex-encoded amaterasu transaction
  amaterasu-tx [options] -create [commands]   Create hex-encoded amaterasu transaction
